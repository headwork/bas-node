...
rem              RB_ASIDE (<live>_failed_<stamp>) for a human to inspect.
rem    copy      Forced rollback to an older build. The backup is COPIED so the
rem              original survives for the next rollback, and the current live
rem              is kept as a NEW backup rather than as a failure.
rem
...
if /i "%RB_MODE%"=="consume" echo [rollback] failed build kept at %RB_ASIDE%
exit /b 0
